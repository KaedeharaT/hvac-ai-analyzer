...
def verify_e2e_smoke():
 ...
 try:
  ...
  valid=(payload.get('evaluation_type')=='local_qwen_end_to_end' and payload.get('runner_successful') is True and payload.get('provider')=='local_qwen' and payload.get('case_count',0)>=50 and payload.get('real_llm_calls',0)>0 and float(metrics.get('Average LLM Latency',0))>0 and E2E_REQUIRED_CATEGORIES <= categories)
  return {'status':'PASS','evidence':f"real Local-Qwen artifact: {payload['case_count']} cases, {payload['real_llm_calls']} calls"} if valid else {'status':'FAIL','evidence':'missing real Local-Qwen E2E evidence, latency, coverage, or artifact'}
