...
def test_deterministic_agent_evaluation_has_sixty_meaningful_runtime_cases(tmp_path):
    ...
    assert {'Total Cases','Routing Accuracy','Tool Selection Accuracy','Task Success Rate','Grounded Answer Rate','Hallucination Rate','Abstention Accuracy','Tool Failure Rate','Average Tool Calls','Average Agent Latency','Average LLM Latency','RAG Retrieval Hit Rate'} == set(result['metrics'])
    assert result['metrics']['Total Cases'] >= 60
    assert result['metrics']['Routing Accuracy'] == 1.0
    assert result['metrics']['Hallucination Rate'] == 0.0
