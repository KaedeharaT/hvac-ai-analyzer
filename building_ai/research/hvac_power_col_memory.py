...
from building_ai.llm.client import chat_json
from building_ai.config.compat import DIRECT_PROMPT_VERSION, LLM_MODEL
from building_ai.research.direct_prompt_templates import (
    DIRECT_LABELS,
    build_direct_prompt,
    validate_direct_prompt_version,
)
...
def local_llm_chat_json(prompt: str):
    ...
    return chat_json(
        prompt,
        system_msg="あなたはHVACデータの専門家です。必ずJSONで回答してください。",
        temperature=0.0,
        seed=0,
    )
...
- "heat_source": 主機 / 熱源ユニット（例: ヒートポンプ, 冷凍機, ボイラー）に関係する列なら高いスコア。
  - 典型的なキーワード: "AHP", 独立した "HP", "chiller", "冷凍機", "熱源", "冷温水機", "主機", "室外機" など。
  - 注意: "WSHP", "ASHP", "VRF" はこのデータでは末端側設備として優先的に扱い、"HP" を含むだけで heat_source を高くしない。
- "terminal": 室内機・末端空調機（AHU, FCU, 室内機など）に関係する列なら高いスコア。
  - キーワード: "WSHP", "ASHP", "VRF", "AHU", "FCU", "室内機", "fan coil", "吹出温度", "吸込温度", "室温", "zone", "VAV" など。
...
- "吸込" / "吹出" は主に末端空調機の空気側 supply/return の手がかりであり、単独では heat_source の根拠にしない。
- "還流" / "往流" は、"冷水" / "冷温水" / "温水" / "熱源" / "チラー" / "冷凍機" / "ポンプ" など水側・熱源側の語が同時にある場合のみ heat_source の根拠にする。
- "外気温度" / "温度差" / "温度設定" / "水槽温度" はロール体系外または補助指標の可能性が高いため、C1 では heat_source を高くしない。
...
def score_slots_for_label( df,
    col,
    series,
    label,
    slot_llm=None,
    unit_infer=None,
    use_physical_validation=True,
    use_abstention=True,):
    name = str(col)
    low = name.lower()
    ...
    # ==== C1: 组件类型 ====
    c1_heat = float(c1.get("heat_source", 0.0) or 0.0)
    c1_terminal = float(c1.get("terminal", 0.0) or 0.0)
    c1_fan = float(c1.get("fan", 0.0) or 0.0)

    c1_terminal_side = bool(re.search(r"WSHP|ASHP|VRF|FCU|AHU|室内機", name, re.I))
    c1_air_side = any(k in name for k in ["吸込", "吹出"])
    c1_water_heat_source_context = bool(
        re.search(r"冷水|冷温水|温水|熱源|チラー|chiller|冷凍機|ポンプ|pump", name, re.I)
    )
    c1_water_direction = any(k in name for k in ["還流", "往流"])
    c1_other_protected = bool(
        re.search(r"外気温度|外氣温度|温度差|温度設定|水槽温度", name, re.I)
    )

    if c1_other_protected:
        c1_heat = 0.0
        c1_terminal = 0.0
        c1_fan = 0.0
    elif c1_terminal_side or (c1_air_side and not c1_water_heat_source_context):
        c1_terminal = max(c1_terminal, c1_heat, c1_fan, 0.8)
        c1_heat = min(c1_heat, 0.1)
        c1_fan = min(c1_fan, 0.3)
    elif c1_water_direction and not c1_water_heat_source_context:
        c1_heat = min(c1_heat, 0.1)
    ...
    s2 = 0.0
    if need_power:
        # 日算値/積算/電力量/kWh 是累计能量，不允许判成瞬时 power
        if _DAILY_TOKEN.search(name):
            detail = dict(C1=0.0, C2=0.0, C3=0.0, C4=0.0,
                          C5=0.0, C6=0.0, C7=0.0, C8=0.0, total=0.0)
            return 0.0, detail
    ...
    # ==== C8: 物理妥当性 ====
    raw_s8 = float(c8.get("ok", 0.0) or 0.0)
    s8 = raw_s8 if use_physical_validation else 0.0
    ...
    detail = dict(
        ...
        C8=s8, C8_raw=raw_s8,
        physical_validation_enabled=bool(use_physical_validation),
        total=total,
    )
    return total, detail
...
def local_llm_semantic_attributes(col_name: str):
    ...
    - equipment_type: heat_source / terminal / electrical / pump / non_hvac / unknown
    - hvac_related: bool
    - cop_relevance: bool
    """
    key = str(col_name)
    if key in _SEMANTIC_LLM_CACHE:
        return _SEMANTIC_LLM_CACHE[key]

    prompt = f"""
You are an HVAC and building energy data expert.

Given one BEMS column name, classify its semantic attributes.
...
Definitions:
- physical_type:
  - "power" means instantaneous power, usually kW or W.
  - "energy" means accumulated electrical energy, usually kWh, Wh, 電力量, 積算, 日算値.
  - "temperature" means temperature.
  - "flow" means water flow or air volume.
- equipment_type:
  - "heat_source": heat pump, chiller, boiler, outdoor unit, heat source system.
  - "terminal": AHU, FCU, indoor unit, air-conditioning terminal equipment.
  - "electrical": lighting, socket, main feeder, distribution panel, shop/cafe electrical load.
  - "pump": pump load, drainage pump, circulation pump.
  - "non_hvac": clearly not HVAC related.
- hvac_related:
  - true if the variable belongs to HVAC equipment or HVAC operation.
  - false for lighting, sockets, main feeder, shops, cafe loads, general electrical loads, etc.
- cop_relevance:
  - true ONLY when this column can be used as heat-source instantaneous input power for COP calculation.
  - false for energy/kWh, terminal equipment power, lighting power, socket power, pump power, main feeder power, and non-HVAC loads.
  - If unsure, set cop_relevance=false.

Important:
- 電力 = instantaneous power.
- 電力量 = accumulated energy, not instantaneous power.
- COP requires heat-source instantaneous power, not energy and not terminal/non-HVAC power.
"""
    ...
    default = {
        ...
        "cop_relevance": False,
        "confidence": 0.0,
        "reason": "LLM failed or uncertain"
    }
    ...
    equipment_type = str(obj.get("equipment_type", "unknown")).strip().lower()

    if physical_type not in {"temperature", "flow", "power", "energy", "pressure", "capacity", "other"}:
        physical_type = "other"
    if equipment_type not in {"heat_source", "terminal", "electrical", "pump", "non_hvac", "unknown"}:
        equipment_type = "unknown"
    ...
    # 最小物理边界：COP 只能用 heat_source + instantaneous power
    cop_relevance = bool(obj.get("cop_relevance", False))
    if physical_type != "power" or equipment_type != "heat_source":
        cop_relevance = False

    result = {
        "physical_type": physical_type,
        "equipment_type": equipment_type,
        "hvac_related": bool(obj.get("hvac_related", False)),
        "cop_relevance": cop_relevance,
        "confidence": confidence,
        "reason": str(obj.get("reason", "")),
    }

    _SEMANTIC_LLM_CACHE[key] = result
    return result



def _direct_basic_stats(series: pd.Series) -> dict:
    s = pd.to_numeric(series, errors="coerce").dropna()
    count = int(len(s))
    if count == 0:
        return {
            "count": 0, "min": None, "max": None,
            "mean": None, "std": None, "zero_ratio": None,
        }
    return {
        "count": count,
        "min": float(s.min()),
        ...
        "zero_ratio": float((s.abs() <= 1e-12).mean()),
    }


def _parse_direct_json(raw):
...
def _stable_direct_digest(value) -> str:
    import hashlib
...
def prepare_direct_prompt(
    ...
    """生成 Direct Prompt 及其稳定输入，不执行模型请求。"""
    version = validate_direct_prompt_version(
        prompt_version or DIRECT_PROMPT_VERSION
    )
    needs_series = version != "direct_v1_name"
    if needs_series and series is None:
        raise ValueError(f"{version} 需要传入当前列的 pandas Series")

    sample_values = None
    stats = None
    selected_neighbors = None
    if needs_series:
        sample_values = _robust_sample_values(
            pd.Series(series), k=40, prefer_nonzero=True, seed=0
        )
    if version in {"direct_v3_name_values_stats", "direct_v4_full_context"}:
        stats = _direct_basic_stats(pd.Series(series))
    if version == "direct_v4_full_context":
        if neighbor_cols is None:
            print(f"[DIRECT PROMPT WARNING] {version} 未提供 neighbor_cols，将使用空列表")
        current_raw = str(
            matched_column_name if matched_column_name is not None else col_name
        )
        current_normalized = normalize_column_name_for_matching(current_raw)
        selected_neighbors = [
            str(name) for name in list(neighbor_cols or [])
            if (
                str(name) != current_raw
                and normalize_column_name_for_matching(name) != current_normalized
            )
        ][:30]
    ...
    prompt = build_direct_prompt(
        ...
        stats=stats,
        neighbor_cols=selected_neighbors,
    )
    return {
        "version": version,
        "prompt": prompt,
...
def local_llm_name_role(
    ...
    """
    prompt_input = prepare_direct_prompt(
        col_name=col_name,
        series=series,
    ...
    sample_values = prompt_input["sample_values"]
    stats = prompt_input["stats"]
    selected_neighbors = prompt_input["neighbor_cols"]

    key = (
        str(col_name),
        version,
        _stable_direct_digest(sample_values),
        _stable_direct_digest(stats),
        _stable_direct_digest(selected_neighbors),
        LLM_MODEL,
    )
    ...
    for i in range(max(1, n_self_consistency)):
        try:
            ...
            if tag_i not in DIRECT_LABELS:
                ...
                parse_statuses[-1] = parse_status
            conf_i = data_i.get("confidence", 0.0)
            try:
                conf_i = max(0.0, min(1.0, float(conf_i)))
            except Exception:
                conf_i = 0.0
    ...
    if is_energy_header:
        terminal_like = any(k.lower() in name.lower() for k in [
            "AHU", "FCU", "室内機", "fan", "送風機", "風機", "blower", "air", "VAV"
        ])

        heat_source_like = any(k.lower() in name.lower() for k in [
            "AHP", "HP", "熱源", "冷凍機", "冷温水機", "チラー", "chiller",
            "boiler", "ボイラ", "R-", "REF", "CH", "CHW"
        ])

        if heat_source_like and not terminal_like:
            tag = "heat_source_energy"
        elif terminal_like:
            tag = "terminal_energy"
        else:
            # 不知道设备类型时，默认先放 heat_source_energy
            # 后面可通过分组或人工确认修正
            tag = "heat_source_energy"
    ...
    result = {
        ...
        "parse_status": (
            "ok" if parse_statuses and all(s == "ok" for s in parse_statuses)
            else ",".join(parse_statuses) if parse_statuses else "request_failure"
        ),
...
def infer_physical_type_from_name(col_name: str) -> str:
    name = str(col_name)
    low = name.lower()
    ...
    # energy 必须优先于 power
    if any(k in name for k in ["電力量", "積算", "日算値", "日算", "日積算"]) or re.search(r"\bkwh\b|\bwh\b", low):
        return "energy"

    if any(k in name for k in ["温度", "水温", "外気温度"]) or "℃" in name or "°c" in low:
        return "temp"

    if any(k in name for k in ["流量", "風量"]) or any(k in low for k in ["m3/h", "m³/h", "l/min", "l/s", "air volume", "flow"]):
        return "flow"

    if any(k in name for k in ["圧力", "高圧", "低圧"]) or any(k in low for k in ["kpa", "mpa", "pressure"]):
        return "pressure"

    if "電力" in name or re.search(r"\bkw\b", low):
        return "power"
...
def infer_device_type_from_name(col_name: str) -> str:
    name = str(col_name)
    low = name.lower()

    terminal_keywords = [
        "WSHP", "ASHP", "VRF", "AHU", "FCU", "室内機", "照明", "誘導灯", "コンセント",
        "売店", "カフェ", "排水ポンプ", "主幹", "fan", "送風機", "VAV"
    ]

    heat_source_keywords = [
        "AHP", "熱源", "冷凍機", "冷温水機",
        "チラー", "chiller", "boiler", "ボイラ"
    ]

    if any(k.lower() in low for k in terminal_keywords):
        return "terminal"

    if any(k.lower() in low for k in heat_source_keywords):
        return "heat_source"

    if re.search(r"(^|[^A-Za-z0-9])HP([^A-Za-z0-9]|$)", name, re.I):
        return "heat_source"

    return "unknown"


def candidate_labels_by_type_and_device(physical_type: str, device_type: str) -> list[str]:
    def _ordered(heat_label: str, terminal_label: str) -> list[str]:
        if device_type == "terminal":
            return [terminal_label, heat_label, "other"]
        if device_type == "heat_source":
            return [heat_label, terminal_label, "other"]
        return [heat_label, terminal_label, "other"]

    if physical_type == "energy":
        return _ordered("heat_source_energy", "terminal_energy")

    elif physical_type == "power":
        return _ordered("heat_source_power", "terminal_power")

    elif physical_type == "flow":
        return _ordered("heat_source_flow", "terminal_air_volume")

    elif physical_type == "temp":
        if device_type == "terminal":
            candidates = [
            ...
            candidates = [
                "heat_source_supply_temp", "heat_source_return_temp",
                "terminal_supply_air_temp", "terminal_return_air_temp", "other"
            ]
        return candidates

    elif physical_type == "capacity":
        return _ordered("heat_source_capacity", "terminal_capacity")
...
def infer_role_by_slots(
    ...
    def _finalize_and_return(label, score, per_label, debug):
        debug.setdefault("label_after_validation", label)
        if debug.get("label_after_validation") is None:
            debug["label_after_validation"] = label
    ...
    def _detail_c8(label):
        detail = per_label.get(label, {}) if isinstance(per_label, dict) else {}
        if "C8_raw" in detail:
            return float(detail.get("C8_raw") or 0.0)
        return float(detail.get("C8", 0.0) or 0.0)
    ...
    # === 1. 分层候选过滤：先锁定 physical_type / device_type，再只对候选标签打分 ===
    best_label = "other"
    best_score = 0.0
    per_label: dict[str, dict] = {}

    physical_type = infer_physical_type_from_name(col)
    device_type = infer_device_type_from_name(col)
    candidate_labels = candidate_labels_by_type_and_device(physical_type, device_type)

    debug["physical_type"] = physical_type
    debug["device_type"] = device_type
    debug["candidate_labels"] = candidate_labels

    print(f"[CANDIDATES] {col} type={physical_type}, device={device_type}, candidates={candidate_labels}")
    ...
    for lab in candidate_labels:
        ...
        sc, det = score_slots_for_label(
            ...
            use_physical_validation=use_physical_validation,
    ...
    debug["slot_score"] = slot_best_score
    ...
    if (not use_local_llm_direct) or (local_llm_tag is None) or (local_llm_conf is None):
        if slot_best_score < tau_low:
            ...
            debug.update({
                ...
                "label_before_validation": slot_best_label,
                "label_after_validation": "other",
                "decision": "other",
                "suspicious_flag": True,
                "explain_zh": msg_zh,
        ...
        debug.update({
            ...
            "final_score": slot_best_score,
            "label_before_validation": slot_best_label,
            "label_after_validation": slot_best_label,
            "decision": "slot",
            "suspicious_flag": False,
            "explain_zh": msg_zh,
            "explain_ja": msg_ja,
    ...
    if q_conf_adj < name_llm_high:
        ...
        debug.update({
            ...
            "c8_best": c8_best,
            "C8": c8_best,
            "label_before_validation": label_before_validation,
        ...
        if use_physical_validation and c8_best < gate_min_c8:
            gate_status = "ABSTAIN"
            gate_reason = f"low_physical_validity(C8={c8_best:.2f}<{gate_min_c8})"
            debug["validation_reason"] = gate_reason
        elif use_abstention and eff_gap < gate_min_gap and max(eff_q, eff_s) < gate_strong:
            gate_status = "ABSTAIN"
            gate_reason = f"ambiguous_evidence(eff_gap={eff_gap:.2f}<{gate_min_gap})"
            debug["abstention_reason"] = gate_reason
        ...
        if gate_status == "ABSTAIN" and local_llm_tag != "other":
            ...
            if strong_name_hint:
                ...
                debug.update({
                    ...
                    "label_after_validation": soft_label,
    ...
    debug["C8"] = c8_best
    debug["label_before_validation"] = slot_best_label
    gate_min_c8 = 0.35
    if use_physical_validation and c8_best < gate_min_c8:
        msg_zh = (
        ...
        debug.update({
            ...
            "gate_status": "ABSTAIN",
            "gate_reason": f"low_physical_validity(C8={c8_best:.2f}<{gate_min_c8})",
            "validation_reason": f"low_physical_validity(C8={c8_best:.2f}<{gate_min_c8})",
            "suspicious_flag": True,
            "explain_zh": msg_zh,
            "explain_ja": msg_ja,
        })
...
def batch_physical_role_review(
    ...
    use_slot_features = bool(ablation_config.get("use_slot_features", True))
    use_physics_reasoning = bool(ablation_config.get("use_physics_reasoning", True))
    use_physical_validation = bool(ablation_config.get("use_physical_validation", True))
    use_abstention = bool(ablation_config.get("use_abstention", True))
    ablation_name = ablation_config.get("name", "full")
    print(f"[ABLATION] mode={ablation_name} slot={use_slot_features} physics_reasoning={use_physics_reasoning} "
          f"physical_validation={use_physical_validation} abstention={use_abstention}")
    ...
    for col in df.columns:
        ...
        label, score, per_label, dbg = infer_role_by_slots(
            ...
            slot_module_enabled=use_slot_features,
            use_physical_validation=use_physical_validation,
            use_abstention=use_abstention,
        )
        dbg["physical_type_llm"] = semantic_info.get("physical_type")
        dbg["equipment_type_llm"] = semantic_info.get("equipment_type")
        dbg["hvac_related"] = semantic_info.get("hvac_related")
        dbg["cop_relevance"] = semantic_info.get("cop_relevance")
        dbg["physics_confidence"] = semantic_info.get("confidence")
        dbg["semantic_reason"] = semantic_info.get("reason")
        dbg["physics_reasoning_enabled"] = use_physics_reasoning
        dbg["slot_module_enabled"] = use_slot_features
        dbg["physical_validation_enabled"] = bool(use_physical_validation and use_slot_features)
        dbg["abstention_enabled"] = bool(use_abstention and use_slot_features)

        # Physics Reasoning 仅记录语义/物理告警，不修改语义预测标签。
        if use_physics_reasoning:
            physical_type = semantic_info.get("physical_type")
            equipment_type = semantic_info.get("equipment_type")
            hvac_related = semantic_info.get("hvac_related", False)
            cop_relevance = semantic_info.get("cop_relevance", False)
            physics_warnings = []

            if not hvac_related:
                physics_warnings.append("non_hvac")

            if physical_type == "energy" and "power" in str(label):
                physics_warnings.append("energy_not_power")

            if physical_type in ["pressure", "other"] and label != "other":
                physics_warnings.append("invalid_physical_type")

            if label == "heat_source_power" and not cop_relevance:
                physics_warnings.append("not_cop_relevant")

            if (
                (str(label).startswith("heat_source") and equipment_type == "terminal")
                or (str(label).startswith("terminal") and equipment_type == "heat_source")
            ):
                physics_warnings.append("device_mismatch")
            dbg["physics_reasoning_enabled"] = use_physics_reasoning
            dbg["physics_reasoning_decision"] = "metadata_only"
            dbg["physics_warning"] = physics_warnings
            dbg["semantic_physical_type"] = semantic_info.get("physical_type")
            dbg["semantic_equipment_type"] = semantic_info.get("equipment_type")
            dbg["semantic_hvac_related"] = semantic_info.get("hvac_related")
            dbg["semantic_cop_relevance"] = semantic_info.get("cop_relevance")
            dbg["semantic_confidence"] = semantic_info.get("confidence")

        # 若 infer_role_by_slots 返回 other 且插槽很低，可以考虑兜底关键字/AI
        # 但如果 gate 明确 ABSTAIN（不允许推理），则不要用关键字兜底强行贴标签。
        if label == "other" and score < 0.3 and dbg.get("gate_status") != "ABSTAIN" and dbg.get("suspicious_flag") is not True:
    ...
    # 导出 CSV
    if export_slot_csv and csv_rows is not None:
        base_dir = Path(__file__).resolve().parents[2] / "data" / "debug" / "slot"
        base_dir.mkdir(parents=True, exist_ok=True)
        out_path = slot_csv_path or str(base_dir / "slot_scores_latest.csv")
...
def flatten_slot_debug_fields(per_label, debug):
    """
    Export-only helper: flatten already-computed slot/fusion diagnostics.
    It must not be used by prediction logic.
    """
    per_label = per_label if isinstance(per_label, dict) else {}
    ...
    row = {
        "candidate_labels": json.dumps(debug.get("candidate_labels", []), ensure_ascii=False),
        ...
        "label_before_validation": debug.get("label_before_validation"),
        "label_after_validation": debug.get("label_after_validation"),
        "gate_status": debug.get("gate_status"),
        "gate_reason": debug.get("gate_reason"),
        "validation_reason": debug.get("validation_reason"),
        "abstention_reason": debug.get("abstention_reason"),
        "C8": debug.get("C8"),
        "c8_best": debug.get("c8_best"),
        "is_temp_like": debug.get("is_temp_like"),
    }

    component_names = ["C1", "C2", "C3", "C4", "C5", "C6", "C7", "C8"]
    for comp in component_names:
        best_label = None
        best_score = None
        for label, detail in per_label.items():
            if not isinstance(detail, dict):
                continue
            value = detail.get(comp)
            try:
                score = float(value)
            except (TypeError, ValueError):
                continue
            if best_score is None or score > best_score:
                best_label = label
                best_score = score
        row[f"{comp}_top_label"] = best_label
        row[f"{comp}_top_score"] = best_score

    for label, detail in per_label.items():
        if not isinstance(detail, dict):
            continue
        safe = _safe_label(label)
        row[f"label_{safe}_total"] = detail.get("total", 0.0)
        for comp in component_names:
            row[f"label_{safe}_{comp}"] = detail.get(comp, 0.0)
        if "C8_raw" in detail:
            row[f"label_{safe}_C8_raw"] = detail.get("C8_raw")
...
def build_slot_debug_row(col, final_label, final_score, per_label, debug):
    ...
    row = {
        ...
        "c8_best": debug.get("c8_best"),
        "is_temp_like": debug.get("is_temp_like"),
        "physical_type_llm": debug.get("physical_type_llm"),
        "equipment_type_llm": debug.get("equipment_type_llm"),
        "hvac_related": debug.get("hvac_related"),
        "cop_relevance": debug.get("cop_relevance"),
        "semantic_reason": debug.get("semantic_reason"),
    }
    row.update(flatten_slot_debug_fields(per_label, debug))
